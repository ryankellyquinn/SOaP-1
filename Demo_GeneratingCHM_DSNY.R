...
byFileAOP("DP3.30015.001", site="DSNY", year="2014", check.size=T);
...
for (time in 2014:2014) 
{
  for (i in cd1)
  {
    for (j in cd2)
    {
      inputpath=sprintf("DP3.30015.001/%i/FullSite/D03/%i_DSNY_1/L3/DiscreteLidar/CanopyHeightModelGtif/%i_DSNY_1_%i_%i_CHM.tif",time,time,time,i,j);
...
write.csv(head,file=sprintf("/usr3/graduate/wangytj/GE585-Share/DSNY_CHM_%i.CSV",time))